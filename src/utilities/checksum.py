...
def generate_checksum( logger: logging.Logger , artifact_path: Path ) -> str :
	"""
	...
	logger.debug( f"Starting checksum generation for {artifact_path.name} using {CHECKSUM_ALGORITHM}" )
	...
	logger.info( f"Generated {CHECKSUM_ALGORITHM} checksum for {artifact_path.name}: " )

	return final_checksum


def load_checksum_history( logger: logging.Logger ) -> Set[ str ] :
	"""
...
def save_checksum( logger: logging.Logger , checksum: str ) -> None :
	"""
	...
	try :
		...
		logger.error( f"Permission denied writing to checksum history: {permission_error}" )
